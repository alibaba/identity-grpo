...
DIMENSION_DESCRIPTIONS = {
    ...
    'SA': ['Subject Alignment', 'the alignment between the faces in the video and the faces in the reference images'],
}
...
SUBJECT_ALIGNMENT_PROMPT_WITH_SPECIAL_TOKEN = """
Your task is to evaluate the quality of the video generated based on the all above reference images in the Subject alignment criteria. Please provide a rating from 0 to 10 for Subject alignment criteria, with 0 being the worst and 10 being the best. 

**Subject Alignment:**  
Evaluate the degree of similarity between the human face in the generated video and the human face in all the reference images above.
The following sub-dimensions should be considered:
1. **Per-Face Similarity (Highest Priority)**  
   For each reference face, evaluate whether it appears in the video and how closely it matches, in this order:  
   a. **Facial structure and expression** – eye shape, nose, mouth, jawline, symmetry, emotion — *most important*  
   b. **Hairstyle, hair color, and facial hair**  
   c. **Skin tone and complexion**  
   d. **Clothing and accessories** (e.g., glasses, hats) — *only supportive clues*

   → A single strong facial match (especially in structure and expression) outweighs multiple weak or superficial matches.

Please provide the rating of subject alignment: <|SA_reward|>
END
"""
# SUBJECT_ALIGNMENT_PROMPT_WITH_SPECIAL_TOKEN = """
# Your task is to evaluate the quality of the video generated based on the all above reference images in the Subject alignment criteria. Please provide a rating from 0 to 10 for each of the three categories, with 0 being the worst and 10 being the best. Each evaluation should be independent of the others.

# **Subject Alignment:**  
# Evaluate the degree of similarity between the human face in the generated video and the human face in all the reference images above.
# The following sub-dimensions should be considered:
# - **Face Visual Details:** Evaluate how the face(s) in the video align with the faces in the above reference images. You should judge the matching degree between the face in the generated video and the face in the reference image from aspects such as face shape, skin tone, eye color, nose shape, mouth shape, etc.
# - **Facial Expression:** Facial expressions are things like smile, laugh, pain, sadness, neutrality, serious, thoughtful, or focused, etc. You should evaluate the degree of match between the facial expressions of the characters in the generated video and those in the reference images.
# - **Appearances:** Appearances are things like pose, contours, hair style, glasses, upper clothes, lower clothes, jumpsuit, shoes, accessories, hat and bags, etc. You should evaluate the degree of match between the appearances of the characters in the generated video and those in the reference images.
# - **Copy-paste Issues** 'Copy-paste Issues' mean directly transfer the face, expression, appearance, pose, and contours from the reference image to the video, resulting in unnatural outcomes. If you determine that the video has obvious' Copy-paste Issues', you should provide this video a worse rating.
...
def build_prompt(prompt, dimension, template_type):
    ...
    elif template_type == "subject_alignment_special":
        return SUBJECT_ALIGNMENT_PROMPT_WITH_SPECIAL_TOKEN
